...
(function() {
  ...
  window.updateCartDisplay = function() {
    ...
    itemsContainer.innerHTML = cartItems.map(function(item, index) {
      return '<div class="cart-item reveal" style="animation-delay: ' + (index * 0.05) + 's">' +
        '<div class="item-image">' + (item.image ? window.renderProductImage(item.image, item.name, { placeholder: '🛍️' }) : (item.emoji || '🛍️')) + '</div>' +
        '<div class="item-details">' +
          '<h4>' + item.name + '</h4>' +
          (item.description ? '<p class="item-description">' + item.description + '</p>' : '') +
          '<p>' + formatARS(item.price) + ' c/u</p>' +
        '</div>' +
        '<div class="item-price">' + formatARS(item.price * item.qty) + '</div>' +
