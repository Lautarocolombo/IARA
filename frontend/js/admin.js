...
async function saveSettings() {
      ...
      document.addEventListener('DOMContentLoaded', () => {
         if (authToken) {
           document.getElementById('loginOverlay').classList.add('hidden');
            const hash = window.location.hash.slice(1);
            const validSections = ['products','categories','orders','reports','testimonials','texts','heroImages','settings'];
            navigateTo(validSections.includes(hash) ? hash : 'reports');
         } else {
    ...
    window.hideSaveStatus = hideSaveStatus;
    window.clearSaveStatus = clearSaveStatus;
    ...
    async function saveToCloud(section, action, btnId, loadingId, defaultText, loadingText, statusId, successMsg) {
      setButtonState(btnId, loadingId, true, defaultText, loadingText);
      if (statusId) showSaveStatus(statusId, 'saving', 'Guardando cambios...');
      try {
        await action();
        if (statusId) showSaveStatus(statusId, 'success', successMsg || '✅ Guardado');
        window.showToast('✅', successMsg || 'Guardado', 'success');
        return true;
      } catch (err) {
