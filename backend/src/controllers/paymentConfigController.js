  ...
  if (!row) {
    await query(
      `INSERT INTO payment_config (transfer_alias, holder_name, whatsapp, message, active)
       VALUES ($1, $2, $3, $4, $5)`,
      [data.transferAlias || '', data.holderName || '', data.whatsapp || '', data.message || '', data.active !== false]
    );
  } else {
    await query(
      ...
      [data.transferAlias || '', data.holderName || '', data.whatsapp || '', data.message || '', data.active !== false, row.id]
    );
  }
};
